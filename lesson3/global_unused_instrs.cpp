...
int main(int argc, char* argv[]) {
    ...
    // Second iteration through the whole program to eliminate the instruction
    // that has dead variable(s) and has no side effects
    for (int fcnIdx = 0; fcnIdx < brilProg["functions"].size(); ++fcnIdx) {
        Json::Value& brilInstrucions = brilProg["functions"][fcnIdx]["instrs"];
        for (int instrIdx = 0; instrIdx < brilInstrucions.size(); ++instrIdx) {
            if (brilInstrucions[instrIdx].isMember("dest") &&
                (usedVars.find(brilInstrucions[instrIdx]["dest"].asString()) ==
